...
const Modal = ({ user, repos, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="modal-box-custom bg-white p-6 rounded-lg shadow-lg relative max-w-lg w-full overflow-y-auto z-50">
      <button
            ...
            <ul>
              {sortedRepos.map(repo => (
                <li key={repo.id} className="mb-4">
                  <a
                    href={repo.html_url}
                    className="text-blue-500 hover:underline text-lg font-semibold z-50"
                    target="_blank"
                    rel="noopener noreferrer"
                  >
                    {repo.name}
